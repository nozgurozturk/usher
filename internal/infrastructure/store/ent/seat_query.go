...
func (sq *SeatQuery) QueryTickets() *TicketQuery {
	query := &TicketQuery{config: sq.config}
...
func (sq *SeatQuery) QueryRows() *RowQuery {
...
func (sq *SeatQuery) WithRows(opts ...func(*RowQuery)) *SeatQuery {
	query := &RowQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *SeatQuery) sqlAll(ctx context.Context) ([]*Seat, error) {
	...
	if query := sq.withTickets; query != nil {
		...
		query.Where(predicate.Ticket(func(s *sql.Selector) {
			s.Where(sql.InValues(seat.TicketsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.seat_tickets
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "seat_tickets" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "seat_tickets" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := sq.withRows; query != nil {
		...
		nodeids := make(map[uuid.UUID][]*Seat)
		for i := range nodes {
			if nodes[i].row_seats == nil {
				continue
			}
			fk := *nodes[i].row_seats
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		}
		query.Where(row.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "row_seats" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Rows = n
			}
		}
	}
