...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withTickets:      uq.withTickets.Clone(),
...
func (uq *UserQuery) WithTickets(opts ...func(*TicketQuery)) *UserQuery {
	query := &TicketQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withTickets = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withReservations; query != nil {
		...
		query.Where(predicate.Reservation(func(s *sql.Selector) {
			s.Where(sql.InValues(user.ReservationsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_reservations" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_reservations" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Reservations = append(node.Edges.Reservations, n)
		}
	}

	if query := uq.withTickets; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Ticket(func(s *sql.Selector) {
			s.Where(sql.InValues(user.TicketsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_tickets
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_tickets" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_tickets" returned %v for node %v`, *fk, n.ID)
			}
