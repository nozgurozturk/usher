...
func (eq *EventQuery) QueryReservations() *ReservationQuery {
...
func (eq *EventQuery) QueryTickets() *TicketQuery {
	query := &TicketQuery{config: eq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EventQuery) QueryLayout() *LayoutQuery {
	query := &LayoutQuery{config: eq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EventQuery) Clone() *EventQuery {
	...
	return &EventQuery{
		...
		withLayout:       eq.withLayout.Clone(),
...
func (eq *EventQuery) WithReservations(opts ...func(*ReservationQuery)) *EventQuery {
	query := &ReservationQuery{config: eq.config}
...
func (eq *EventQuery) WithLayout(opts ...func(*LayoutQuery)) *EventQuery {
	query := &LayoutQuery{config: eq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (eq *EventQuery) sqlAll(ctx context.Context) ([]*Event, error) {
	...
	if query := eq.withReservations; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Reservations = []*Reservation{}
		}
		query.withFKs = true
		query.Where(predicate.Reservation(func(s *sql.Selector) {
			s.Where(sql.InValues(event.ReservationsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.event_reservations
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "event_reservations" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "event_reservations" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Reservations = append(node.Edges.Reservations, n)
		}
	}
	...
	if query := eq.withTickets; query != nil {
		...
		query.Where(predicate.Ticket(func(s *sql.Selector) {
			s.Where(sql.InValues(event.TicketsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.event_tickets
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "event_tickets" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "event_tickets" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Tickets = append(node.Edges.Tickets, n)
		}
	}

	if query := eq.withLayout; query != nil {
		ids := make([]uuid.UUID, 0, len(nodes))
		nodeids := make(map[uuid.UUID][]*Event)
		for i := range nodes {
			if nodes[i].layout_events == nil {
				continue
			}
			fk := *nodes[i].layout_events
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "layout_events" returned %v`, n.ID)
			}
			for i := range nodes {
