...
func (tq *TicketQuery) QuerySeat() *SeatQuery {
	query := &SeatQuery{config: tq.config}
...
func (tq *TicketQuery) sqlAll(ctx context.Context) ([]*Ticket, error) {
	...
	if tq.withSeat != nil || tq.withEvent != nil || tq.withUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := tq.withSeat; query != nil {
		...
		for i := range nodes {
			if nodes[i].seat_tickets == nil {
				continue
			}
			fk := *nodes[i].seat_tickets
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "seat_tickets" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := tq.withEvent; query != nil {
		...
		nodeids := make(map[uuid.UUID][]*Ticket)
		for i := range nodes {
			if nodes[i].event_tickets == nil {
				continue
			}
			fk := *nodes[i].event_tickets
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "event_tickets" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := tq.withUser; query != nil {
		...
		nodeids := make(map[uuid.UUID][]*Ticket)
		for i := range nodes {
			if nodes[i].user_tickets == nil {
				continue
			}
			fk := *nodes[i].user_tickets
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
